...
import { Leaf, TrendingUp, TrendingDown, Minus, Eye, Droplets } from 'lucide-react';
...
const AgromonitoringIndicators: React.FC<AgromonitoringIndicatorsProps> = ({
  ...
  const getNdviStatus = (value: number) => {
    if (value >= 0.6) return 'Vegetación Densa';
    if (value >= 0.4) return 'Vegetación Moderada';
    if (value >= 0.2) return 'Vegetación Escasa';
    return 'Suelo/Sin Vegetación';
  };
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Índices de Vegetación</CardTitle>
        </CardHeader>
        <CardContent>
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm">Índices de Vegetación Satelital</CardTitle>
          <div className="flex items-center gap-2">
            ...
            {data?.measurement_date && (
              <span className="text-xs text-muted-foreground">
                {new Date(data.measurement_date).toLocaleDateString('es-ES')}
              </span>
            )}
          </div>
        </div>
      ...
      <CardContent>
        ...
        <div className="mt-4 pt-4 border-t">
          <p className="text-xs font-medium mb-2">Interpretación NDVI:</p>
          <div className="flex flex-wrap gap-4 text-xs text-muted-foreground">
            <span><span className="text-green-600">●</span> ≥0.6: Vegetación densa</span>
            <span><span className="text-yellow-600">●</span> 0.3-0.6: Moderada</span>
            <span><span className="text-red-600">●</span> &lt;0.3: Escasa/Suelo</span>
          </div>
        </div>
        ...
        {data?.satellite_image_url && (
          <div className="mt-4 pt-4 border-t">
            <p className="text-xs font-medium mb-2">Imagen Satelital (NDVI):</p>
            <img 
